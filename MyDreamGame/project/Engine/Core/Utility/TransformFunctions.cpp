...
Matrix4x4 TransformFunctions::Inverse(const Matrix4x4 &matrix) {
	DirectX::XMMATRIX xmMat = DirectX::XMLoadFloat4x4(reinterpret_cast<const DirectX::XMFLOAT4X4*>(&matrix));
	DirectX::XMVECTOR det;
	DirectX::XMMATRIX xmInv = DirectX::XMMatrixInverse(&det, xmMat);
	Matrix4x4 result;
	DirectX::XMStoreFloat4x4(reinterpret_cast<DirectX::XMFLOAT4X4*>(&result), xmInv);
	return result;
}
...
// 2.正射影行列
Matrix4x4 TransformFunctions::MakeOrthographicMatrix(float left, float top, float right, float bottom, float nearClip, float farClip) {
	Matrix4x4 result = {};
...
Matrix4x4 TransformFunctions::MakeRotateMatrix(const Quaternion& quaternion) {
...
Matrix4x4 TransformFunctions::MakeAffineMatrix(const Vector3& scale, const Quaternion& rotate, const Vector3& translate) {
    Matrix4x4 scaleMatrix = MakeScaleMatrix(scale);
...
Vector3 TransformFunctions::Lerp(const Vector3& v1, const Vector3& v2, float t) {
    return {
        v1.x + t * (v2.x - v1.x),
        v1.y + t * (v2.y - v1.y),
        v1.z + t * (v2.z - v1.z)
