...
namespace {
    ...
    void BuildModelPreviewTriangles(ModelSelectItem& item, float boxSize, ImVec4 baseColor) {
        ...
        std::sort(item.cachedTriangles.begin(), item.cachedTriangles.end(), [](const ModelPreviewTriangle& a, const ModelPreviewTriangle& b) {
            return a.avgZ < b.avgZ;
        });
    }

    bool DrawCachedModelPreview(ImDrawList* drawList, ImVec2 center, const ModelSelectItem& item) {
...
void ModelSelectModal::Initialize() {
    RefreshModelList();
}

void ModelSelectModal::EnsureModelLoaded(ModelSelectItem& item) {
...
void ModelSelectModal::RefreshModelList() {
    modelList_.clear();
    selectedModelIdx_ = -1;
    ...
    // 重複除去
    std::vector<ModelSelectItem> uniqueList;
...
void ModelSelectModal::DrawModal(const char* popupId, bool& isOpen, const std::function<void(const ModelSelectItem&)>& onSelect) {
    if (!isOpen) return;
    ...
    ImGui::SetNextWindowSize(ImVec2(760.0f, 540.0f), ImGuiCond_Appearing);

    bool modalOpen = true;
    if (ImGui::BeginPopupModal(popupId, &modalOpen, ImGuiWindowFlags_NoScrollbar)) {
        if (!modalOpen) {
            isOpen = false;
        ...
        // --- 1. ヘッダー (タイトル・検索バー・件数) ---
        ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "リソースから3Dモデルを選択して追加");
        ImGui::SameLine();
        ...
        ImGui::InputTextWithHint("##Filter", "モデル名・ファイル名で検索...", searchFilter_, sizeof(searchFilter_));
        ...
        for (int i = 0; i < static_cast<int>(modelList_.size()); ++i) {
            ...
            std::string itemLower = item.displayName + " " + item.fileName + " " + item.directoryPath;
        ...
        // --- 2. カード一覧エリア (ChildWindow) ---
        float footerHeight = 52.0f;
        ImGui::BeginChild("ModelCardsRegion", ImVec2(0, -footerHeight), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
        ...
        bool doubleClickedItem = false;
        int doubleClickedIdx = -1;
        ...
        for (size_t fIdx = 0; fIdx < filteredIndices.size(); ++fIdx) {
            ...
            if (isHovered) {
                ...
                ImGui::Spacing();
                ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "ダブルクリックで即座に追加");
                ImGui::EndTooltip();
            }
        ...
        bool hasSelection = (selectedModelIdx_ >= 0 && selectedModelIdx_ < static_cast<int>(modelList_.size()));
        std::string selText = hasSelection ? ("選択中: " + modelList_[selectedModelIdx_].displayName + " (" + modelList_[selectedModelIdx_].fileName + ")") : "モデルを選択してください";
        ImGui::TextColored(hasSelection ? ImVec4(1.0f, 0.85f, 0.3f, 1.0f) : ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "%s", selText.c_str());

        ImGui::SameLine(ImGui::GetWindowWidth() - 220.0f);
        if (!hasSelection) ImGui::BeginDisabled();
        if (ImGui::Button("このモデルを追加", ImVec2(120, 26)) || (doubleClickedItem && doubleClickedIdx >= 0)) {
            int targetIdx = doubleClickedItem ? doubleClickedIdx : selectedModelIdx_;
            if (targetIdx >= 0 && targetIdx < static_cast<int>(modelList_.size())) {
                if (onSelect) {
                    onSelect(modelList_[targetIdx]);
                }
                isOpen = false;
                ImGui::CloseCurrentPopup();
            }
        ...
        ImGui::SameLine();
        if (ImGui::Button("キャンセル", ImVec2(80, 26))) {
            isOpen = false;
            ImGui::CloseCurrentPopup();
        }
