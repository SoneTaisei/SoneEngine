...
void LiftBlock::Initialize(ID3D12Device* device, Primitive* boxPrimitive, float worldX, float worldY, float width, float height) {
    ...
    // 繝ｪ繝輔ヨ縺ｮ濶ｲ・井ｾ具ｼ壽・繧九＞鮟・牡繧・が繝ｬ繝ｳ繧ｸ・・
    ...
    // 繝ｬ繝ｼ繝ｫ縺ｮ遽・峇繧呈爾邏｢縺励※遘ｻ蜍慕ｯ・峇繧呈ｱｺ螳壹☆繧・
    ...
    if (hasHorizontalRail) {
        direction_ = { 1.0f, 0.0f, 0.0f }; // 豌ｴ蟷ｳ遘ｻ蜍・
        // 螳滄圀縺ｮ遘ｻ蜍輔・荳ｭ蠢・ｽ咲ｽｮ縺九ｉ縺ｪ縺ｮ縺ｧ隱ｿ謨ｴ
        ...
        direction_ = { 0.0f, 1.0f, 0.0f }; // 蝙ら峩遘ｻ蜍・
        ...
        direction_ = { 0.0f, 0.0f, 0.0f }; // 繝ｬ繝ｼ繝ｫ縺後↑縺・ｴ蜷医・蜍輔°縺ｪ縺・
...
void LiftBlock::Update() {
    ...
    switch (state_) {
        ...
        case LiftState::WaitingAtEnd:
            ...
            if (waitTimer_ >= waitTime_) { // waitTime_蠕・ｩ・
    ...
    if (state_ == LiftState::MovingForward) {
        // 蜉騾溷ｺｦ繝代Λ繝｡繝ｼ繧ｿ繧剃ｽｿ逕ｨ縺励◆繧､繝ｼ繧ｸ繝ｳ繧ｰ (Power Ease In)
        progress = std::pow(currentT_, acceleration_);
    } else {
        // 蠕ｩ霍ｯ縺翫ｈ縺ｳ蠕・ｩ滓凾縺ｯLinear
        progress = currentT_;
    }
